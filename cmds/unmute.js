...
module.exports = {
  ...
  async execute(context) {
    ...
    if (!targetId) return context.reply('📖 /unmute @user');

    database.query(
      'DELETE FROM mutes WHERE peer_id = ? AND user_id = ?',
      [peerId, targetId],
      async (err) => {
        ...
        if (context.logAction) context.logAction('/unmute', targetId);
        context.reply(`✅ Мут снят\n\n${targetLink}\nАдмин: ${adminLink}`);
      }
    );
  }
};
